...
import { spuService,skuService } from '@/services';

export const createSpu = async (req: CustomRequest, res: Response, next: NextFunction) => {
    const spu = await spuService.newSpu({
        ...req.body,
        product_shop: req?.user?.userId as string
    })
    new successResponse.CREATED({
        message: "Created a new spu",
        metadata: spu
    }).send(res);
}

export const findSku = async (req: CustomRequest, res: Response, next: NextFunction) => {
    console.log('query',req.query)
   const {sku_id,product_id} = req.query as {sku_id:any,product_id:any}
...
export const findSpu = async (req: CustomRequest, res: Response, next: NextFunction) => {
    console.log('query',req.query)
   const {product_id} = req.query as {product_id:any}
